...
func TestTestHandlers(t *testing.T) {
	...
	testCases := []struct{
		...
		},
		{
			...
			fmt.Sprintf("/%s/", db.Tests),
			http.StatusAccepted,
			[]byte(fmt.Sprintf(`{"command":"test","name":"test1","assignment_def":"1:%d:ass","runs_on":0}`, year)),
			testUsers[users.Admin],
			nil,
		},
		{
		...
		},
		{
			...
			fmt.Sprintf("/%s/", db.Tests),
			http.StatusAccepted,
			[]byte(fmt.Sprintf(`{"command":"test","name":"test1","assignment_def":"1:%d:ass","runs_on":0,"files":{"elements":{}}}`, year)),
			testUsers["user1"],
			nil,
		},
		{
		...
		},
		{
			...
			fmt.Sprintf("/%s/", db.Tests),
			http.StatusAccepted,
			[]byte(fmt.Sprintf(`{"command":"test","name":"test1","assignment_def":"1:%d:ass","runs_on":1,"files":{"elements":{}}}`, year)),
			testUsers["user2"],
			nil,
		},
		{
