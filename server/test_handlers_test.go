...
func TestTestHandlers(t *testing.T) {
	...
	testCases := []struct{
		...
		},
		{
			...
			fmt.Sprintf("/%s/", db.Tests),
			http.StatusAccepted,
			[]byte(fmt.Sprintf(`{"timeout":1,"command":"test","name":"test1","assignment_def":"1:%d:ass","runs_on":0}`, year)),
			testUsers[users.Admin],
			nil,
		},
		{
		...
		},
		{
			...
			fmt.Sprintf("/%s/1/%d/ass/test1", db.Tests, year),
			http.StatusAccepted,
			[]byte(fmt.Sprintf(`{"created_by":"admin","created_on":"2021-06-24T22:37:49.321616Z","updated_by":"admin","updated_on":"2021-06-24T22:37:49.321616Z","timeout":1,"name":"test1","assignment_def":"1:%d:ass","runs_on":1,"files":{"elements":{"file.txt":{}}},"state":0}`, year)),
			testUsers[users.Admin],
			nil,
		},
		{
		...
		},
		{
			...
			fmt.Sprintf("/%s/", db.Tests),
			http.StatusAccepted,
			[]byte(fmt.Sprintf(`{"timeout":1,"command":"test","name":"test1","assignment_def":"1:%d:ass","runs_on":0,"files":{"elements":{}}}`, year)),
			testUsers["user1"],
			nil,
		},
		{
		...
		},
		{
			...
			fmt.Sprintf("/%s/1/%d/ass/test1", db.Tests, year),
			http.StatusAccepted,
			[]byte(fmt.Sprintf(`{"timeout":1,"name":"test1","assignment_def":"1:%d:ass","runs_on":1,"files":{"elements":{"file.txt":{}}}}`, year)),
			testUsers["user1"],
			nil,
		},
		{
		...
		},
		{
			...
			fmt.Sprintf("/%s/", db.Tests),
			http.StatusAccepted,
			[]byte(fmt.Sprintf(`{"timeout":1,"command":"test","name":"test1","assignment_def":"1:%d:ass","runs_on":1,"files":{"elements":{}}}`, year)),
			testUsers["user2"],
			nil,
		},
		{
