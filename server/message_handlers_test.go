...
func TestMessageHandlers(t *testing.T) {
	...
	req, err = http.NewRequest(http.MethodPost, fmt.Sprintf("/%s/", db.Tests), bytes.NewBuffer([]byte(fmt.Sprintf(`{"timeout":1,"command":"test","assignment_def":"1:%d:ass","runs_on":1,"name":"test"}`, year))))
	if err != nil {
		panic(err)
	}
